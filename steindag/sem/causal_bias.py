...
class CausalBiasMixin:
    ...
    def _causal_bias(
        ...
        outcome_means = self._cond_mean_outcome(u_latent_samples, observed, outcome_name=outcome_name)
        causal_bias = 0.
            ...
            def causal_bias_contrib_sample(u_latent_sample: dict[str, Tensor]) -> Tensor:
                ...
                def causal_bias_contrib_sample_per_observation(
                    ...
                    if observed_name == treatment_name:
                        ...
                        dx_diff = grad(fv_bar_x)(observed_per_observation[treatment_name])
                    ...
                    return -(du_fy + mid_term) * dx_diff / self._variables[observed_name].sigma
    ...
    def _get_u_observed(
        self, u_latent: dict[str, Tensor], observed: dict[str, Tensor], observed_name: str
    ) -> Tensor:
        values: dict[str, Tensor] = {}
            ...
            def cond_mean_outcome_per_observation(
                ...
                for name, variable in self._variables.items():
                    ...
                    if name in observed_per_observation:
                        ...
                        values[name] = variable.f(parents, u_latent_per_observation[name])
